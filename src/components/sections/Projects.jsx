        ...
        <FadeIn delay={0}>
          <div className='text-center mb-12'>
            <div className='inline-flex items-center gap-2 px-4 py-2 bg-primary/10 border border-primary/30 rounded-full mb-6'>
              ...
              <span className='text-sm text-primary font-medium'>Meu trabalho</span>
            </div>
            <h2 className='text-4xl lg:text-5xl font-normal text-white mb-4'>Projetos Feitos</h2>
            <p className='text-lg text-white/60 max-w-2xl mx-auto'>Meus melhores projetos estão aqui!</p>
          </div>
        </FadeIn>
            ...
            {categories.map(category => (
              ...
              >
                <div className={`absolute inset-0 rounded-full transition-all duration-300 ${activeCategory === category
                  ? 'bg-primary/10 opacity-100' : 'bg-white/5 border border-white/10 group-hover:bg-white/10'
                }`} />
                ...
                {Array.from({ length: Math.max(0, filteredProjects.length - 2) }).map((_, index) => (
                  <button
                    ...
                    className={`transition-all duration-300 rounded-full ${index === currentIndex
                      ? 'bg-primary w-6 h-2' : 'bg-white/30 w-2 h-2 hover:bg-white/50'}`}
